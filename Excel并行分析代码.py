...
if __name__ == "__main__":
    ...
    # target_date_y = datetime.datetime(2024,2,24).strftime("%Y-%m-%d")
    
    # output path
    folder_path = 'C:/Users/cyjiang/Documents/债券预警模型/增量/'
    output_path = folder_path + "预警并行结果分析" + target_date + ".xlsx"
    # input path dict
...
# -------------------------------------------------------------------------
    # 1. 系统 主体预警得分，等级 当日
    sys_data = pd.read_pickle(output_dict['model']+"entity_res_t.pkl")
    sys_df = pd.DataFrame(sys_data)
    
    # 2. 新模型 主体预警得分，等级 当日
    new_data = pd.read_pickle(output_dict['model']+"warn_entityscore.pkl")
    new_df = new_data[new_data['日期'] == target_date] # 现在新模型数据已经开始并行了，需要筛选日期了
    
    # 3. 新模型 主体input coef，传导关系
    new_input_data = pd.read_pickle(output_dict['model']+"rela_pd_level.pkl")
    new_input_df = pd.DataFrame(new_input_data)
    
    # 4. 过渡模型 主体预警得分，等级
    trans_data = pd.read_pickle(output_dict['transform']+'trans_amount.pkl')
    trans_df = pd.DataFrame(trans_data)
    trans_df = trans_df[['主体名称','旧模型新分数','预警等级_trans','预警得分_trans']]

    # 5. 滑档区间
    all_interval_data = pd.read_pickle(output_dict['model']+'all_intervals.pkl')
    all_intervals_df = pd.DataFrame(all_interval_data)

    # 4. 系统 过去三十天的记录
    sys_30_data = ib.get_Entity_sys_t(target_date, db_info)
    sys_past30_df = pd.DataFrame(sys_30_data)
    # 5. 新模型 过去三十天的记录
    new_past30_df = new_data[new_data['日期'] > # 以后如果是读取过往记录的话就要改下逻辑，会把target date之后的都读入
...
# Convert each column
for column in all_intervals_df.columns:
    min_values = all_intervals_df[column]
    max_values = all_intervals_df[column].shift(-1)
    # Use the next row's value as max, fill the last with its own value

    # Format intervals as [min, max) and round to 3 decimal places
    all_intervals_df[column] = ['[' + format(min_value, '.3f') + ', ' + format(max_value, '.4f') + ')' for
                            min_value, max_value in zip(min_values, max_values)]

all_intervals_df = all_intervals_df.iloc[:-1, :]

with pd.ExcelWriter(output_path, engine='openpyxl', mode='a') as writer:
    all_intervals_df.to_excel(writer, sheet_name='滑档区间')
...
level_summary_df['预警等级_new-关联关系传导前'] = level_summary_df['等级'].map(new_input_df['final_level'].value_counts()).fillna(0).astype(int)
...
# # 添加'主体名称'
# comparison_df['主体名称'] = sys_df['主体名称'].unique()
...
for sheet in workbook.sheetnames:
    ...
    for col in worksheet.columns:
            ...
            adjusted_width = (max_length + 5)  # 添加些许额外空间
...
# -------------------------------------结论注释-------------------------------------------
#
anno1 = '结论分析：主体预警等级在关联关系传导后，正常风险等级（绿色）的主体的预警等级会轻微上移，但这不会影响到风险等级的比例。'
anno2 = '平滑过渡说明：过渡期模型第一天分数和等级与系统分数对标；根据不同初始等级，主体在各自对应的衰减周期完成后，预警等级会与新模型保持一致。'
anno3 = '衰减周期：初始系统预警等级<3: 30天；3-5: 90天；>5: 180天。'

sheets_to_include = ["预警等级分布"]
for sheet_name in workbook.sheetnames:
    if sheet_name in sheets_to_include:
        datesheet = workbook[sheet_name]
    # 将注释添加
    datesheet['A14'] = anno1
    datesheet['A15'] = anno2
